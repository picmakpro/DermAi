...
// -----------------------------
// Canonical values (EN) + UI labels
// -----------------------------

// Canonical concerns (EN) used for logic/storage
...
type CanonicalConcern = typeof CANONICAL_CONCERNS[number]

// UI labels for concerns (display only)
const CONCERN_LABELS: Record<CanonicalConcern, string> = {
...
// Canonical routine products (EN) – UI + storage
...
// Display helpers for canonical user profile values
const DISPLAY_GENDER_EN: Record<string, string> = {
  male: 'Male',
  female: 'Female',
  other: 'Other',
  'prefer-not-to-say': 'Prefer not to say',
}
const DISPLAY_SKIN_TYPE_EN: Record<string, string> = {
  dry: 'Dry',
  normal: 'Normal',
  combination: 'Combination',
  oily: 'Oily',
  sensitive: 'Sensitive',
  unknown: 'To be determined by AI',
}

// Routine type options (canonical EN)
const ROUTINE_OPTIONS = [
  { value: 'minimalist', label: 'Minimalist', help: '2–3 essential steps' },
  { value: 'simple', label: 'Simple', help: '3–4 easy steps' },
  { value: 'balanced', label: 'Balanced', help: '4–5 optimized steps' },
  { value: 'complete', label: 'Complete', help: '5–7 detailed steps' },
] as const
...
interface UserProfile {
  age: number
  gender: string // canonical EN (male|female|other|prefer-not-to-say)
  skinType: string // canonical EN (dry|normal|combination|oily|sensitive|unknown)
}

interface SkinConcerns {
  primary: string[] // canonical EN concerns
}

interface CurrentRoutine {
  morningProducts: string[] // EN product names
  eveningProducts: string[] // EN product names
  monthlyBudget: string // canonical EN (under-50|50-100|100-200|over-200|no-limit)
  routinePreference?: string // canonical EN (minimalist|simple|balanced|complete)
}

interface QuestionnaireData {
  userProfile: UserProfile
  skinConcerns: SkinConcerns & {
    otherText: string // free text when "other" selected
  }
...
export default function SkinQuestionnaire() {
  ...
  // Initialize with canonical EN defaults
  const [data, setData] = useState<QuestionnaireData>({
    ...
    currentRoutine: {
      morningProducts: [],
      eveningProducts: [],
      monthlyBudget: '50-100', // canonical EN via normalizeBudget will also accept
    },
    allergies: {
      ingredients: [],
      pastReactions: '',
  ...
  const updateData = (
    section: keyof QuestionnaireData,
    updates: Partial<QuestionnaireData[keyof QuestionnaireData]>
  ) => {
    setData(prev => ({
      ...prev,
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Data already canonical EN; keep normalization for robustness
      const completeData = {
        ...
        skinConcerns: {
          ...data.skinConcerns,
          primary: data.skinConcerns.primary,
        },
        currentRoutine: {
          ...data.currentRoutine,
          routinePreference: data.currentRoutine.routinePreference,
          monthlyBudget: data.currentRoutine.monthlyBudget,
        },
        allergies: data.allergies,
      }

      console.log('✅ Canonical questionnaire data stored:', completeData)
      ...
      // Fallback, but still store what we have
      const completeData = {
        photos: JSON.parse(photosData),
  ...
  // Toggle helpers (store canonical EN)
  const toggleArrayItem = (array: string[], item: string, max?: number) => {
    if (array.includes(item)) {
      // if unselecting "other", clear its text
      if (item === 'unknown') setShowAiMessage(false)
      if (item === 'other') updateData('skinConcerns', { otherText: '' })
      ...
      const filtered = array.filter(i => i !== 'unknown')
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 6:
        return true
      case 7:
        return !!data.currentRoutine.routinePreference
      default:
        return true
    }
  }
  ...
  // Gender button (convert any imported label to canonical EN via normalizer)
  const onSelectGender = (option: string) => {
    const canonical = normalizeGender(option) || option
    updateData('userProfile', { gender: canonical })
  }

  // Skin type button (convert any imported label to canonical EN via normalizer)
  const onSelectSkinType = (type: string) => {
    const canonical = normalizeSkinType(type) || type
    updateData('userProfile', { skinType: canonical })
  }

  // Routine preference
  const onSelectRoutinePreference = (value: string) => {
    const canonical = normalizeRoutinePreference(value) || value
    updateData('currentRoutine', { routinePreference: canonical })
  }

  // Budget (if constants are FR like "50-100€", normalize to EN)
  const onSelectBudget = (rangeLabel: string) => {
    const canonical = normalizeBudget(rangeLabel) || rangeLabel
    updateData('currentRoutine', { monthlyBudget: canonical })
  }

  // Display helpers

  const hasRoutineProducts = () =>
    data.currentRoutine.morningProducts.length > 0 ||
    data.currentRoutine.eveningProducts.length > 0

  const getRoutineDisplay = () => {
    const { morningProducts: morning, eveningProducts: evening } = data.currentRoutine
    if (morning.length === 0 && evening.length === 0) return 'No routine'
    const parts: string[] = []
    if (morning.length > 0) parts.push(`Morning: ${morning.join(', ')}`)
    if (evening.length > 0) parts.push(`Evening: ${evening.join(', ')}`)
    return parts.join(' • ')
  }

  const getConcernsDisplay = () => {
    if (data.skinConcerns.primary.includes('unknown')) return 'AI will analyze automatically'
    let concerns = [...data.skinConcerns.primary]
    if (concerns.includes('other') && data.skinConcerns.otherText.trim()) {
      concerns = concerns.filter(c => c !== 'other')
      concerns.push(data.skinConcerns.otherText.trim())
    }
    // Map canonical to labels
    return concerns
      .map(c => (CONCERN_LABELS as any)[c] || c)
      .join(', ')
  }
    ...
    <div className="card bg-dermai-pure border border-dermai-nude-200 rounded-2xl p-5 shadow-premium">
      ...
      <div className="space-y-2 text-sm text-gray-700">
        ...
        <div className="flex items-center">
          ...
          <span className="text-gray-700">
            {data.userProfile.gender !== 'prefer-not-to-say' &&
              `${DISPLAY_GENDER_EN[data.userProfile.gender] || data.userProfile.gender}, `}
            {selectedAgeRange || `${data.userProfile.age} years`}
            {` • Skin ${
              DISPLAY_SKIN_TYPE_EN[data.userProfile.skinType] ||
        ...
        {data.skinConcerns.primary.length > 0 && (
          <div className="flex items-start">
            ...
            <div className="text-gray-700">
              {data.skinConcerns.primary.includes('unknown') ||
              data.skinConcerns.primary.length === 0 ? (
        ...
        <div className="flex items-start">
          ...
          <span className="text-gray-700 text-xs">
            ...
            {data.currentRoutine.routinePreference
              ? `• Preference: ${
                  ROUTINE_OPTIONS.find(o => o.value === data.currentRoutine.routinePreference)?.label ||
                  data.currentRoutine.routinePreference
                }`
              : ''}
          </span>
        ...
        <div className="flex items-center">
          ...
          <span className="text-gray-700">
            Budget {data.currentRoutine.monthlyBudget}
          </span>
        </div>
        ...
        {data.allergies.pastReactions.trim() && (
          <div className="flex items-start text-sm">
            ...
            <div className="text-gray-700 text-xs">
              <span className="font-medium">Past reactions:</span>
              <br />
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-2xl lg:text-3xl font-bold font-display text-dermai-neutral-900 mb-2">
                Personal profile
              </h2>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold font-display text-dermai-neutral-800 mb-3">
                  Age *
                </label>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {GENDER_OPTIONS.map(option => {
                    const canonical = normalizeGender(option) || option
                    const isSelected = data.userProfile.gender === canonical
                    const label =
                      DISPLAY_GENDER_EN[canonical] || option
                    return (
                      <button
                        key={option}
                        type="button"
              ...
              <div>
                <label className="block text-sm font-semibold font-display text-dermai-neutral-800 mb-3">
                  Skin type (if you know it)
                </label>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {SKIN_TYPES.map(type => {
                    const canonical = normalizeSkinType(type) || type
                    const isSelected = data.userProfile.skinType === canonical
                    const label =
                      DISPLAY_SKIN_TYPE_EN[canonical] || canonical
                    return (
                      <button
                        key={type}
                        type="button"
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-dermai-neutral-600">
                What are your main skin concerns? (max 3) *
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {CANONICAL_CONCERNS.map(concern => (
                  <button
                    key={concern}
                    onClick={() =>
                      updateData('skinConcerns', {
                        primary: toggleArrayItem(
                          data.skinConcerns.primary,
                          concern,
                          3
                        ),
                      })
                  ...
                  >
                    {CONCERN_LABELS[concern as CanonicalConcern]}
                  </button>
                ))}
              </div>
            ...
            {data.skinConcerns.primary.includes('other') && (
              <div className="bg-dermai-ai-50 border border-dermai-ai-200 rounded-xl p-4">
                ...
                <input
                  ...
                  onChange={e =>
                    updateData('skinConcerns', { otherText: e.target.value })
                  }
                ...
                <div className="flex">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 mt-1">
                      The AI will analyze your photos and automatically estimate
                      your priority concerns with high accuracy.
                    </p>
                  </div>
                </div>
              </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-dermai-neutral-600">
                Which products do you use? (optional)
              </p>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {COMMON_PRODUCTS.map(product => (
                    <button
                      key={`morning-${product}`}
                      onClick={() =>
                        updateData('currentRoutine', {
                          morningProducts: toggleArrayItem(
                            data.currentRoutine.morningProducts,
                            product
                          ),
                        })
                      }
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {COMMON_PRODUCTS.map(product => (
                    <button
                      ...
                      onClick={() =>
                        updateData('currentRoutine', {
                          eveningProducts: toggleArrayItem(
                            data.currentRoutine.eveningProducts,
                            product
                          ),
                        })
                      }
              ...
              <div>{/* Budget moved to step 7 */}</div>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-dermai-neutral-600">
                Any ingredients to avoid? (optional)
              </p>
            </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateData('allergies', {
                        ingredients: toggleArrayItem(
                          data.allergies.ingredients,
                          ingredient
                        ),
                      })
                    }
            ...
            {data.allergies.ingredients.length > 0 &&
              !data.allergies.ingredients.includes('No known allergies') && (
                <div>
                  <label className="block text-sm font-semibold font-display text-dermai-neutral-800 mb-2">
                    Past reactions (optional)
                  </label>
                  <textarea
                    value={data.allergies.pastReactions}
                    onChange={e =>
                      updateData('allergies', { pastReactions: e.target.value })
                    }
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-dermai-neutral-600">
                Last preferences to personalize your recommendations
              </p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {ROUTINE_OPTIONS.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => onSelectRoutinePreference(opt.value)}
                    className={`p-3 text-left rounded-xl border-2 transition-all hover-lift ${
                      data.currentRoutine.routinePreference === opt.value
                        ? 'border-dermai-ai-500 bg-dermai-ai-50 text-dermai-ai-700 shadow-glow'
                        : 'border-dermai-nude-200 bg-dermai-pure text-dermai-neutral-700 hover:border-dermai-ai-300 hover:bg-dermai-ai-50'
                    }`}
                  >
                    <div className="font-semibold text-dermai-neutral-900">
                      {opt.label}
                    </div>
                    <div className="text-xs text-dermai-neutral-600 mt-1">
                      {opt.help}
                    </div>
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {BUDGET_RANGES.map(range => {
                  const canonical = normalizeBudget(range) || range
                  const isSelected = data.currentRoutine.monthlyBudget === canonical
                  return (
                    <button
                      key={range}
                      type="button"
                      onClick={() => onSelectBudget(range)}
                      className={`p-3 text-sm rounded-xl border-2 transition-all hover-lift ${
                        isSelected
                          ? 'border-dermai-ai-500 bg-dermai-ai-50 text-dermai-ai-700 shadow-glow'
                          : 'border-dermai-nude-200 bg-dermai-pure text-dermai-neutral-700 hover:border-dermai-ai-300 hover:bg-dermai-ai-50'
      ...
      <div className="bg-dermai-pure/80 backdrop-blur-sm border-b border-dermai-nude-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            {/* Progress dots - 7 visible steps */}
            <div className="hidden md:flex items-center space-x-2">
              {[...Array(7)].map((_, i) => (
                <div
                  key={i}
                  className={`w-2.5 h-2.5 rounded-full transition-all ${
                    i <= currentStep ? 'bg-dermai-ai-500 shadow-glow' : 'bg-dermai-neutral-300'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="text-right">
              <div className="text-sm text-dermai-neutral-500">
                Question {Math.max(1, currentStep)}/{totalSteps}
              </div>
